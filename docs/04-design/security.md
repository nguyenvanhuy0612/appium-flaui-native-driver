...
| :--- | :--- | :--- |
| `power_shell` (the `powershell` script + `appium:prerun`) | `flauinative:power_shell` | **Arbitrary code execution** on the host, with the server's privileges. Runs out-of-process (not on the UIA watchdog) but is otherwise unrestricted; bounded only by the per-call `timeout` (default 300 000 ms — [timeout reference](../02-architecture/stability.md#timeout-reference), axis C). |
